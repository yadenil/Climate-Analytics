...
for country in countries:
     ...
     "cells": [
      {
       ...
       "metadata": {},
       "source": [
        f"# {country.capitalize()} Climate Data Profiling & EDA\n",
        "\n",
        f"This notebook profiles and cleans the climate dataset for {country.capitalize()} from 2015-2026."
       ]
      },
      {
       "cell_type": "code",
       ...
       "source": [
        ...
        "from scipy.stats import zscore\n",
        "\n",
        f"df = pd.read_csv('../data/{country}.csv')\n",
      ...
      },
      {
       ...
       "source": [
        ...
        "df['Date'] = pd.to_datetime(df['YEAR'] * 1000 + df['DOY'], format='%Y%j')\n",
        "df['Month'] = df['Date'].dt.month\n",
        f"df['Country'] = '{country.capitalize()}'\n",
        "df.ffill(inplace=True)\n",
        f"df.to_csv('../data/{country}_clean.csv', index=False)"
       ]
      },
      {
       "cell_type": "markdown",
      ...
      },
      {
       ...
       "source": [
        ...
        f"plt.title('Monthly Avg Temperature - {country.capitalize()} (2015-2026)')\n",
        "plt.show()\n",
        "\n",
        ...
        f"plt.title('Monthly Total Precipitation - {country.capitalize()} (2015-2026)')\n",
        "plt.show()"
       ]
      },
      {
    ...
    with open(f'../notebooks/{country}_eda.ipynb', 'w') as f:
